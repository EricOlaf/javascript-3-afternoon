...
let evenNumbers = mixedNumbers.filter(checkEven)
...
function taxPrices (elem){
  return elem * 1.07;
}
let postTaxPrices = prices.map(taxPrices);
...
let totalPopulation = populations.reduce(function(total, curElement){
...
let myStrongest = monstersInYourPocket.filter(strong);
...
let ordersTotal = orders.reduce(function(total, obj){
  return total + obj.price + obj.tax;
}, 0)
...
let bobsTotal = purchases.reduce(function(total, obj){
 if (obj.owner === "Bob"){return total + obj.pice;} 
}, 0) 
